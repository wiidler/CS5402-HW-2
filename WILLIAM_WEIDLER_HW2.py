...
class DecisionTree:
    ...
    def _split_data(
        self, data: np.ndarray, feature_idx: int, feature_val: float
    ) -> tuple:
        """
    ...
    def _partition_entropy(self, g1_labels: list, g2_labels: list) -> float:
        ...
        return self._data_entropy(g1_labels) * (
            len(g1_labels) / total_count
        ) + self._data_entropy(g2_labels) * (len(g2_labels) / total_count)
    ...
    def find_best_cutpoint(self, data: np.ndarray, attribute_list: list) -> tuple:
        ...
        # Sort the feature values from lowest to highest
        data.sort(axis=0)

        # Iterate through the successive midpoints of the sorted feature values
        information_gain = 0
        feature_idx = 0
        cutpoint = 0
        ...
        for idx in attribute_list:
            for i in range(len(data) - 1):
                feature_val = (data[i][idx] + data[i + 1][idx]) / 2
                g1_labels = data[data[:, idx] < feature_val][:, -1]
                g2_labels = data[data[:, idx] >= feature_val][:, -1]
                info_gain = self._data_entropy(data[:, -1]) - self._partition_entropy(
                    g1_labels, g2_labels
                )
                if info_gain > information_gain:
                    information_gain = info_gain
                    feature_idx = idx
                    cutpoint = feature_val
...
def main():
    train_data = np.load(
        "./train_data.npy", allow_pickle=True
    )  # [112, 5], each sample contains 4 features with the last column as label.

    my_dt = DecisionTree()
    feature_idx, feature_val, information_gain = my_dt.find_best_cutpoint(
        train_data, [0, 1, 2, 3]
    )
    print(
        "The {0}th feature should be used at the tree root with the spliting value: {1}. The infomation gain by this particition is {2}".format(
            feature_idx + 1, feature_val, information_gain
        )
    )
